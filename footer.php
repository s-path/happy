<footer class="footer">
      <section class="footer-pc_section">
        <ul class="footer-pc_wrapper" style="padding-top:95px;">
          <img class="footer-pc_wrapper_img" src="<?php bloginfo('template_directory'); ?>/images/67siteheader.png" alt="" >
          <li><a style="border-top: 1px solid rgba(0,0,0,0.2);" class="footer-pc_wrapper_list01" href="/">ホーム</a></li>
          <li><a class="footer-pc_wrapper_list01" href="/greeting/">初めての方へ</a></li>
        </ul>
        <ul class="footer-pc_wrapper">
        <li><a class="footer-pc_wrapper_list02" href="/features/">整体院Happyが選ばれる<br>５つの理由</a></li>
          <li><a class="footer-pc_wrapper_list02" href="/bodycare/">整体</a></li>
          <li><a class="footer-pc_wrapper_list02" href="/maps/">アクセス・駐車場</a></li>
        </ul>
        <ul class="footer-pc_wrapper01">
          <p class="footer-pc_wrapper01_text">〒377-0008 群馬県渋川市渋川1771-8(川島内科クリニック様 隣)</p>
          <img class="footer-pc_wrapper01_img" src="https://shibukawa-massage-happy.com/wp-content/uploads/2021/07/yoyakuimg.png" alt="" style="background-color:#66ff00">
          <div style="position:relative;">
            <li><a class="footer-pc_wrapper01_access" href="#" style="cursor: default;" >ネット予約
            <span style="position:absolute;font-size: 12px;top: 43px;max-width: 150px;text-align: center;line-height: 1.2;left: 20px;">整備中の為、お電話でのご予約をお願い致します</span>
            </a></li>
            <a href="https://instagram.com/maeno.seitai.happy?utm_medium=copy_link" style="position:absolute; right:5px;top:8px; height:100%; width:46%;" >
              <img src="https://shibukawa-massage-happy.com/wp-content/uploads/2021/07/insta01.png" alt="" style="width:170px;" >
            </a>
          </div>
        </ul>
      </section>

      <ul class="footer-list">
        <li><a href="/">ホーム ></a></li>
        <li><a href="/greeting/">初めての方へ ></a></li>
        <li><a href="/features/">整体院Happyが選ばれる<br>５つの理由 ></a></li>
        <li><a href="/bodycare/">整体 ></a></li>
        <li><a href="/maps/">アクセス・駐車場 ></a></li>
        <li class="footer-list__reserve"><a href="#" class="footer-list__reserve-link">ネット予約 >
          <span class="footer-list__note">整備中の為、お電話でのご予約をお願い致します</span></a></li>
      </ul>
      <div class="footer-text">
        <p>当ウェブサイトに掲載されているコンテンツは、権利者の許可なく<br>複製、転用等する事は法律で禁止されています。</p>
        <a href="#">プライバシーポリシー</a>
        <p>Copyright (C)happy. All Rights Reserved.</p>
      </div>
    </footer>
    <nav id="footerNav">
      <ul class="footerNav">
        <li class="footerNavList" style="margin-right:40px;"><a href="tel:0279301184"><span class="material-icons footerPhone">phone_in_talk</span> でんわ</a></li>
        <!-- <li class="footerNavList"><a href="/reservation1/"><span class="material-icons footerPhone">domain_verification</span> ネット予約</a></li> -->
        <li class="footerNavList" style="margin-left:40px;"><a href="/maps/"><span class="material-icons footerPhone">share_location</span> アクセス</a></li>
      </ul>
    </nav>
<!-- footer end ------------------------------>
    <?php wp_footer(); ?>
    <script src="<?php echo get_template_directory_uri() ?>/js/main.js" type="text/javaScript" charset="utf-8"></script>
  </body>
</html>
